    ...
    <>
      <nav
        className={`fixed left-0 right-0 z-50 flex md:flex-col lg:flex-row md:gap-4 lg:gap-0 items-center justify-between px-6 py-3 md:px-12 transition-all duration-300 ${
          scrolled
            ? "bg-[#fcfcfc]/90 shadow-md rounded-2xl mx-4 top-4"
      ...
      >
        ...
        {/* SEZIONE MENU */}
        <ul
          className={`absolute left-6 right-6 top-14 flex-col gap-1 p-3 bg-[#fcfcfc] rounded-xl shadow-xl md:static md:flex md:flex-row md:gap-2 md:p-0 md:bg-transparent md:shadow-none ${
            isOpen ? "flex" : "hidden"
          }`}
        >
          {links.map((link) => (
        ...
        <div className="flex-1 flex justify-end">
          <button
            className="inline-flex flex-col justify-center gap-1 w-8 h-8 rounded-full border border-gray-200 bg-[#fcfcfc] cursor-pointer p-1 md:hidden"
            aria-label="Toggle navigation"
            onClick={() => setIsOpen((prev) => !prev)}
          >
            <span className="w-full h-0.5 rounded-full bg-orange-600" />
            <span className="w-full h-0.5 rounded-full bg-orange-600" />
            <span className="w-full h-0.5 rounded-full bg-orange-600" />
          </button>
        </div>
